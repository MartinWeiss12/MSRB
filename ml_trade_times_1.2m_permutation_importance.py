...
COMPLETE_CALL_IND = COMPLETE_CALL_IND_one + COMPLETE_CALL_IND_tw
...
for minutesElapsedCount in np.arange(0.1, 0.3, 0.1):
...
testPlot = plt.figure()
importances_test.plot.box(vert=False, whis=10)
plt.title('TEST SET: Permutation Importances (UNDER 0.3 MINUTES ELAPSED)')
plt.axvline(x=0, color='k', linestyle='--')
plt.xlabel('Decrease in accuracy score')
plt.tight_layout()

trainPlot = plt.figure()
importances_train.plot.box(vert=False, whis=10)
plt.title('TRAINING SET: Permutation Importances (UNDER 0.6 MINUTES ELAPSED)')
plt.axvline(x=0, color='k', linestyle='--')
plt.xlabel('Decrease in accuracy score')
plt.tight_layout()
...
for minutesElapsedCount in np.arange(1.0, 1.6, 0.1):
  minutesElapsedThreshold.append(minutesElapsedCount)
  for i in range(len(binary)):
    if(minutesElapsed[i] < minutesElapsedCount):
      binary[i] = 0
...
axs[0].set_title('TEST SET: Permutation Importances (UNDER 0.6 MINUTES ELAPSED)')
axs[0].axvline(x=0, color='k', linestyle='--')
axs[0].set_xlabel('Decrease in accuracy score')
axs[0].figure.tight_layout()

importances_train.plot.box(vert=False, whis=10, ax = axs[1])
axs[1].set_title('TRAINING SET: Permutation Importances (UNDER 0.6 MINUTES ELAPSED)')
